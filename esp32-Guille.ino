...
void loop() {
  ...
  if ((WiFi.status() == WL_CONNECTED)) { //Check the current connection status
    ...
    http.begin("https://www.googleapis.com/youtube/v3/channels?part=statistics&id=UCr64OZEx8BjIMnph2lg0QiA&key=AIzaSyD6jKvdy-cZ7sAfah5tnFQqYXFKd0pc8D4"); //Specify the URL
    ...
    if (httpCode > 0) { //Check for the returning code
     ...
     if (httpCode == 200){
        ...
        lcd.clear();
        lcd.setCursor(0, 0);
        lcd.print(" kiLLeR415tv @YT");
        Serial.println("kiLLeR415tv@yt");
        lcd.setCursor(0, 1);
